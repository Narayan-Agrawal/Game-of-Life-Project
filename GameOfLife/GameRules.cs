...
namespace GameOfLife
{
    class GameRules
    {
        ...
        private static int CountAliveNeighbours(Grid grid, CoOrdinates coOrdinates)
        {
            ...
            CellCornersEnum enumInnerCell = HelperFactory.GetCellCorner(grid, coOrdinates);
            List<CoOrdinates> reachableCells = new List<CoOrdinates>();
            // populate reachable cells from current cell for easier traversing
            HelperFactory.ReachableCells.TryGetValue(enumInnerCell, out reachableCells);
        ...
        {
            CheckForRowGrowth(inputGrid, outputGrid, -1);
            CheckForRowGrowth(inputGrid, outputGrid, inputGrid.RowCount);
            CheckForColumnGrowth(inputGrid, outputGrid, -1);
            CheckForColumnGrowth(inputGrid, outputGrid, inputGrid.ColumnCount);
        }
        ...
        private static void CheckForColumnGrowth(Grid inputGrid, Grid outputGrid, int colId)
        {
            ...
            {
                if (GameRules.CountAliveNeighbours(inputGrid, new CoOrdinates(i, colId)) == 3)
                {
        ...
        private static void CheckForRowGrowth(Grid inputGrid, Grid outputGrid, int rowId)
        {
            ...
            {
                if (GameRules.CountAliveNeighbours(inputGrid, new CoOrdinates(rowId, j)) == 3)
                {
